...
import { formatScheduleDate } from '@/lib/schedule-format';
...
export default async function AdminCalendarEventsPage() {
  ...
  return (
    <section className="max-w-[960px] mx-auto px-7 pt-14 pb-24">
      ...
      <p className="font-sans text-[13px] text-txt-3 -mt-6 mb-10 max-w-[600px]">
        Calendar entries not tied to a championship round — classification deadlines, reveal
        streams, announcements. Shows on the cumulative calendar (/calendar), and on a sim's own
        calendar too when a sim is set.
      </p>

      {events.length === 0 ? (
        <div className="border border-line/50 bg-carbon-2 px-8 py-12 text-center">
          <p className="font-mono text-[13px] tracking-[.2em] uppercase text-txt-3">No calendar events yet</p>
        </div>
      ) : (
            ...
            <div key={e.id}
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <span className="font-mono text-[10px] tracking-[.2em] uppercase text-txt-3 border border-line px-1.5 py-[1px]">{e.eventType}</span>
                  {e.game && (
                ...
                <span className="font-mono text-[11px] text-txt-3">
                  {formatScheduleDate(e.eventDate)}
                  {e.slug ? ` · ${e.slug}` : ''}
                </span>
              </div>
              <div className="flex items-center gap-4 shrink-0">
                <Link href={`/admin/calendar-events/${e.id}`}
