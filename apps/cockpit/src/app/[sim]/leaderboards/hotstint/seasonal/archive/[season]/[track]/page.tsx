...
// ARCHIVE route for a Hot Stint (Seasonal) track board: finished seasons only (isFrozenSeason).
...
export default async function StintTrackBodyArchive({ params }: { params: Promise<{ sim: string; season: string; track: string }> }) {
  ...
  return <StintTrackBody sim={sim} season={season} trackKey={trackKey} frozen />;
}
