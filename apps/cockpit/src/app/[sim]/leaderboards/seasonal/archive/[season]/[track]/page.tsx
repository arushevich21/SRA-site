...
// ARCHIVE route for a Hot Lap (Seasonal) track board: finished seasons only (isFrozenSeason).
...
export default async function HotlapTrackBodyArchive({ params }: { params: Promise<{ sim: string; season: string; track: string }> }) {
  ...
  return <HotlapTrackBody sim={sim} season={season} trackKey={trackKey} frozen />;
}
