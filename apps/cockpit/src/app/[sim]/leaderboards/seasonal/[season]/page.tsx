...
// LIVE route for Hot Lap (Seasonal) (the current season). Rendered per request: the shell
...
export default async function HotlapSeasonBodyLive({ params }: { params: Promise<{ sim: string; season: string }> }) {
  const { sim: slug, season } = await params;
  const sim = getSimBySlug(slug);
  if (!sim) notFound();
  if (sim.game !== 'ACC') notFound();

  return <HotlapSeasonBody sim={sim} season={season} frozen={isFrozenSeason(season)} />;
}
